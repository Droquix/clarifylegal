"""
ClarifyLegal Legal Guardrail & Disclaimer Service

Applies mandatory non-legal advice disclaimers and sanitizes AI text outputs
to ensure all explanations remain purely informational.
"""
...
def sanitize_informational_text(text: str) -> str:
    ...
    replacing direct legal advice or command phrases with informational alternatives.

    Args:
        text (str): Raw explanation text generated by the AI model.

    Returns:
        str: Sanitized, informational text complying with legal safety guardrails.
    """
    if not text:
        return ""
...
def apply_disclaimer_guardrails(response_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    Applies mandatory disclaimer guardrails recursively across all string fields in an analysis response.

    Args:
        response_dict (Dict[str, Any]): Unchecked JSON response dictionary from AI service.

    Returns:
        Dict[str, Any]: Guardrail-enforced dictionary containing MANDATORY_DISCLAIMER.
    """
    if "summary" in response_dict and isinstance(response_dict["summary"], dict):
        exec_sum: str = response_dict["summary"].get("executive_summary", "")
        response_dict["summary"]["executive_summary"] = sanitize_informational_text(exec_sum)

        rationale: str = response_dict["summary"].get("risk_rationale", "")
    ...
    if "clauses" in response_dict and isinstance(response_dict["clauses"], list):
        for clause in response_dict["clauses"]:
            if isinstance(clause, dict):
                clause["plain_english"] = sanitize_informational_text(str(clause.get("plain_english", "")))
                clause["potential_impact"] = sanitize_informational_text(str(clause.get("potential_impact", "")))
        ...
        for change in response_dict["changes"]:
            if isinstance(change, dict):
                change["plain_english_impact"] = sanitize_informational_text(
                    str(change.get("plain_english_impact", ""))
                )
