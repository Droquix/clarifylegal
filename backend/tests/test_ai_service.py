...
def test_analyze_document_without_provider_key_raises_clear_error(sample_contract_text):
    with patch("backend.services.ai_service.settings.NVIDIA_API_KEY", ""):
        with pytest.raises(HTTPException) as exc_info:
            analyze_document_with_ai(sample_contract_text)
    assert exc_info.value.status_code == 503
    assert "Document analysis is unavailable" in exc_info.value.detail

def test_answer_question_with_ai_missing_key_raises_error(sample_contract_text):
    with patch("backend.services.ai_service.settings.NVIDIA_API_KEY", ""):
        with pytest.raises(HTTPException) as exc_info:
            answer_question_with_ai("What is the rent?", sample_contract_text)
        assert exc_info.value.status_code == 503
        assert "API key is missing" in exc_info.value.detail

def test_answer_question_with_ai_api_error_raises_clear_user_message(sample_contract_text):
    with patch("backend.services.ai_service.settings.NVIDIA_API_KEY", "nvapi-fake-key"):
        with patch("backend.services.ai_service._call_nvidia_api") as mock_api:
            mock_api.side_effect = Exception("API Quota Exceeded")

            with pytest.raises(HTTPException) as exc_info:
                answer_question_with_ai("What is the penalty?", sample_contract_text)

            assert exc_info.value.status_code == 502
            assert "Sorry, document Q&A encountered an error" in exc_info.value.detail

def test_answer_question_with_ai_returns_specific_document_content():
    ...
    mock_ai_json = '{"answer": "The monthly rent is $4,250.", "lawyer_followups": ["Is there a grace period for rent payments?"], "citations": ["Monthly rent is $4,250 payable on the 1st of each month to Landlord Acme Corp."]}'
